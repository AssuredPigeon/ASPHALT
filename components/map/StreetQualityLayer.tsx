...
interface Props {
  calles: CalleEstado[];
  visible: boolean;
  compact?: boolean;  // Solo 1 polyline por calle
}
...
const StreetQualityLayer: React.FC<Props> = ({ calles, visible, compact = false }) => {
    ...
    <>
      {streets.map(s => (
        <React.Fragment key={s.uid}>
          {!compact && <Polyline coordinates={s.coords} strokeColor={s.layers.outer.color} strokeWidth={s.layers.outer.width} lineCap="round" lineJoin="round" geodesic />}
          {!compact && <Polyline coordinates={s.coords} strokeColor={s.layers.mid.color} strokeWidth={s.layers.mid.width} lineCap="round" lineJoin="round" geodesic />}
